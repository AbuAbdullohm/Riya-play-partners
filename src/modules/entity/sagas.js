...
export function* LoadAll(action) {
	const { entity, name, url, params, dataKey, metaKey, appendData, prependData, primaryKey, cb, version = "v1" } = action.payload;

	try {
		const { data } = yield call(api[`request${version}`].get, queryBuilder(url, params));
			...
			Actions.entity.LoadAll.success({
				...
				meta:
					typeof metaKey === "function"
						? metaKey(data)
						: {
								currentPage: get(data, "_meta.currentPage"),
								pageCount: get(data, "_meta.pageCount"),
								perPage: get(data, "_meta.perPage"),
								totalCount: get(data, "_meta.totalCount")
						  }
			})
		);
...
export function* LoadOne(action) {
	const { id, entity, name, url, params, primaryKey, cb, relations, version = "v1" } = action.payload;
...
export function* LoadDefault(action) {
	const { url, params, cb, version = "v1" } = action.payload;
...
export function* Form(action) {
	...
	try {
		const { data } = yield call(api[`request${version}`][method], queryBuilder(url, params), values);
...
export function* FormDefault(action) {
	const { url, params, method, values, cb, version = "v1" } = action.payload;
