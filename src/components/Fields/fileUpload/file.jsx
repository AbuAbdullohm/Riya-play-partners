...
const File = ({ item, isDoc, deleteHandler = () => {} }) => {
	...
	if (Array.isArray(item)) {
		src = get(item, "[0].thumbnails.small.src");
	} else {
		src = get(item, "thumbnails.small.src");
	}
	...
	return isDoc ? (
		<div className="mb-5 mr-5 cursor-pointer zoom-in rounded-md">
			<div className="file-preview rounded-md">
				<div className="file-preview__ext rounded-md">{ext}</div>
				<div>
					<div className="file-preview__title">{description}</div>
					<div className="file-preview__size">{helpers.formatBytes(size)}</div>
				</div>
			</div>
			<div
