...
function getTextFormat(
	...
	switch (format) {
		...
		case "small":
			return (
				<small className={`block mt-1 ${classNames}`}>{children}</small>
			);
		case "strong":
			return (
				<strong className={`block mt-1 ${classNames}`}>
					{children}
				</strong>
			);
        case "bold":
            return (
				<b className={`${classNames}`}>{children}</b>
			);
		case "emphasized":
			return <em className={`${classNames}`}>{children}</em>;
		case "highlighted":
			return (
				<mark className={`bg-yellow-200 p-1 ${classNames}`}>
					{children}
				</mark>
			);
		case "subscript":
